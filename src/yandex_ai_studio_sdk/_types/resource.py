...
from .expiration import ExpirationConfig, ExpirationProtoTypeT
from .misc import is_defined
from .result import BaseProtoResult, ProtoMessageTypeT
...
class BaseResource(BaseProtoResult[ProtoMessageTypeT]):
    id: str
    ...
    @classmethod
    def _kwargs_from_message(cls, proto: ProtoMessageTypeT, sdk: BaseSDK) -> dict[str, Any]:  # pylint: disable=unused-argument
    ...
    def _update_from_proto(self, proto: ProtoMessageTypeT) -> Self:
...
@dataclasses.dataclass(frozen=True)
class BaseDeleteableResource(BaseResource[ProtoMessageTypeT]):
    _lock: asyncio.Lock = dataclasses.field(repr=False)
    _deleted: bool = dataclasses.field(repr=False)

    @classmethod
    def _from_proto(cls, *, sdk: BaseSDK, proto: ProtoMessageTypeT) -> Self:
        return cls(
            _sdk=sdk,
            _lock=asyncio.Lock(),
            _deleted=False,
...
@dataclasses.dataclass(frozen=True)
class ExpirableResource(BaseDeleteableResource[ExpirationProtoTypeT]):
    expiration_config: ExpirationConfig

    @classmethod
    def _kwargs_from_message(cls, proto: ExpirationProtoTypeT, sdk: BaseSDK) -> dict[str, Any]:
